...
float brightness[] = new float[stripNumbers*pixelsPerStrip]; //to make them able to fade slower
...
void draw() {
  ...
  if (testObserver.hasStrips) {
    ...
    //fadeSides  - fadeSides() function: fadeSide(int strip, int side, color c, boolean inOut, int fadeSpeed, int threshold) {

    //Fade in the selected SIDES and fade out all other SIDES... // Doesn't work because it fades the selected side as well!!!
    int selectedSide = floor(map(mouseX, 0, width, 0, 6*3));
    println(selectedSide);
    for (int i = 0; i<strips.size()*sidesPerStrip; i++) {
      if (selectedSide == i) { //Fade in selected strip
        //fadeSide(floor(i/3), i%sidesPerStrip, color(#FF7A00), true, 1, 250);
        println("strip: " +  floor(i/3) + ", side: " + i%sidesPerStrip);
      } else { //Fade out all others (doens't work because it fades the selected side as well!!!
        fadeSide(floor(i/3), i%sidesPerStrip, color(#FF7A00), false, 1, 0); //Min brightness
      }
    }
    //    for (int j=pixelsPerSide*i; j<pixelsPerSide*(i+1); j++) {
    //      fadePixel(j, color(#6ADDFF), true, 20, 10);
    ...
    //Single side
    //if (mousePressed) fadeSide(0, 1, color(#FF7A00), true, 1, 150);
    //if (keyPressed) fadeSide(0, 1, color(#FF7A00), false, 1, 50);


    //All sides
    for (int i = 0; i<strips.size(); i++) {
      for (int j= 0; j < sidesPerStrip; j++) {
        //if (mousePressed) fadeSide(i, j, color(#FF7A00), true, 1, 150); //Max brightness
        //if (keyPressed) fadeSide(i, j, color(#FF7A00), false, 1, 50); //Min brightness
      }
    }


    //fadeSideNew test
    //if (mousePressed) fadeSideNew(5,2, color(#FF7A00), true, 1, 10);

    ////////END OF EXAMPLES///////////////
  }
}

void fadeSide (int strip, int side, color c, boolean inOut, float fadeSpeed, int threshold) {
  for (int i = 0; i<strips.size()*sidesPerStrip; i++) {
    if (side == i) { //Fade in selected side
      for (int j=pixelsPerSide*i; j<pixelsPerSide*(i+1); j++) {
        fadePixel(strip, j, c, inOut, fadeSpeed, threshold);
      }
    }
  }
}
...
void fadePixel(int strip, int pixel, color c, boolean inOut, float fadeSpeed, int threshold) { 
  if (strip == -1) { 
    strip = floor(pixel/(pixelsPerStrip)); //use -1 if you don't declare a specific strip
  }
  ...
  strips.get(strip).setPixel(color(hue(c), saturation(c), brightness[pixel]), pixelNum);
}
